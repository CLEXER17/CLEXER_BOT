...
AEROLINK_API_KEY_13 = os.getenv("AEROLINK_API_KEY_13", "")   # 13th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_14 = os.getenv("AEROLINK_API_KEY_14", "")   # 14th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_15 = os.getenv("AEROLINK_API_KEY_15", "")   # 15th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_16 = os.getenv("AEROLINK_API_KEY_16", "")   # 16th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_17 = os.getenv("AEROLINK_API_KEY_17", "")   # 17th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_18 = os.getenv("AEROLINK_API_KEY_18", "")   # 18th Aerolink key slot — rotated in on further retries, empty until set
AEROLINK_API_KEY_19 = os.getenv("AEROLINK_API_KEY_19", "")   # 19th Aerolink key slot — rotated in on further retries, empty until set
...
_CHAT_SYSTEM_PROMPT = (
    ...
    "garbage characters.\n"
    "- Actively use Telegram HTML formatting in EVERY reply (not just special/long ones) to "
    "make it look polished rather than a flat wall of plain text — look for a genuine chance "
    "to use at least one or two of these in each message:\n"
    "  <b>bold</b> — key terms, numbers, names, the important word(s) in the reply.\n"
    "  <i>italic</i> — asides, notes, disclaimers.\n"
    "  <code>text</code> — inline, for a command/filename/model-name/number/technical term "
    "(e.g. <code>/model</code>, <code>claude-opus-5</code>) — never for full sentences.\n"
    "  <pre>text</pre> — multi-line code, or any content needing fixed-width column alignment "
    "(tables, price/data lists).\n"
    "  <blockquote>text</blockquote> — a quote, or a key takeaway/warning worth visually "
    "setting apart from the rest of the reply.\n"
    "  <tg-spoiler>text</tg-spoiler> — a punchline, twist, or reveal the user would want to "
    "tap to see rather than read immediately (a joke's punchline, a quiz/riddle answer, a "
    "movie/show spoiler).\n"
    "  <a href=\"https://...\">text</a> — whenever you reference a real URL you actually have; "
    "never invent or guess a link.\n"
    "This applies across the whole conversation, every message — not just table-summary or "
    "long replies. Even a short casual reply should still bold the key word or two in it.\n"
...
def _rebuild_schedules():
    ...
    # the dense grid (same 17 minutes/hour added on top for all four kinds,
    # 2026-07-27) + anything relocated onto a fresh minute after a 1:3
    # blacklist — minus whatever's currently blacklisted (a fixed-grid time
    # doesn't stop being generated by _regular_grid just because it got
    # blacklisted, so it has to be explicitly subtracted here or it'd keep
    # firing forever).
    SCAN1_SCHEDULE = sorted((_SCAN_SPECIAL["scan1"] | _regular_grid((2, 23), _SCAN_SPECIAL["scan1"])
                              | _regular_grid(_DENSE_GRID_MINUTES, _SCAN_SPECIAL["scan1"], near_thresh=0)
    ...
    SCAN2_SCHEDULE = sorted((_SCAN_SPECIAL["scan2"] | _regular_grid((7, 27), _SCAN_SPECIAL["scan2"])
                              | _regular_grid(_DENSE_GRID_MINUTES, _SCAN_SPECIAL["scan2"], near_thresh=0)
    ...
    SCAN1_TEST_SCHEDULE = sorted((_SCAN_SPECIAL["test1"] | _regular_grid((9, 27), _SCAN_SPECIAL["test1"])
                                   | _regular_grid(_DENSE_GRID_MINUTES, _SCAN_SPECIAL["test1"], near_thresh=0)
    ...
    SCAN2_TEST_SCHEDULE = sorted((_SCAN_SPECIAL["test2"] | _regular_grid((9, 27), _SCAN_SPECIAL["test2"])
                                   | _regular_grid(_DENSE_GRID_MINUTES, _SCAN_SPECIAL["test2"], near_thresh=0)
...
def _claude_client(kind: str = "btc", attempt: int = 0, scan_ver: int = None, force_aerolink: bool = False, use_aerolink: bool = None):
    ...
    Up to 20 Aerolink key slots (AEROLINK_API_KEY..._20) are supported — on a retry
...
def _aerolink_gw_debug_tag(using_aero: bool, attempt: int, bad_keys: set = None) -> str:
    ...
    of the up-to-20 configured Aerolink key slots this attempt will actually
...
def handle_command(text, chat_id, message=None, sender_id=None, auto=False, _is_special=False, _trigger_hm=None):
    ...
    if cmd in ("/start","/help"):
        ...
        for _kind in ("scan1", "scan2", "demo1", "demo2"):
            ...
            _nt_special = _SCAN_SPECIAL.get(_sched_kind, set())
            ...
            _relocated_now = _SLOT_RELOCATED.get(_kind, set()) - _nt_special
            _regular = (_regular_grid((_ma, _mb), _nt_special)
                        | _regular_grid(_DENSE_GRID_MINUTES, _nt_special, near_thresh=0)
                        | _relocated_now) - _SLOT_BLACKLIST.get(_kind, set())
            _rows = []
        ...
        def _do_scan(cid=chat_id, scan_ver=ver, _auto=auto):
            ...
            try:
                ...
                # kind now, needed for the dense-grid same-coin stacking test
                # (see _is_special_now usage below for the coin-collision skip,
                ...
                for candidate in candidate_order[:MAX_TRIES + 3]:  # a few extras in case of skips
                    ...
                    # stack independent trades on the same coin — each one is its own
...
def _regular_grid(minutes, special: set, near_thresh: int = 5) -> set:
    """Every hour's given minute(s), minus any slot that exactly matches or
    falls within near_thresh minutes of a special time in the same hour
    (avoids running a near-duplicate scan right next to the special one).
    minutes can be a single int or any iterable of ints. near_thresh=0 means
    only an EXACT (h,m) collision with a special time gets excluded — used
    for the dense grid below, which intentionally packs minutes close
    together rather than avoiding the neighborhood of special times."""
...
# Dense test grid — added 2026-07-27 alongside each kind's normal regular
# grid (not replacing it), same 17 minutes for all four kinds (scan1/scan2/
# demo1/demo2), fires simultaneously across all of them. Starts every slot
# as nonspecial (see _ai_category) so nothing here reaches VIP/Free or
# copytrade until a specific (hour,minute) proves itself and gets promoted
# to verified by the existing auto-promote system — same win-rate/1:3-
# blacklist rules as the rest of the regular grid, just denser.
_DENSE_GRID_MINUTES = [3, 7, 9, 13, 15, 19, 23, 27, 29, 33, 37, 41, 45, 48, 52, 56, 59]
...
def _run_test_scan(cid, scan_ver: int, is_special: bool = False, trigger_hm: tuple = None):
    ...
    try:
        ...
        for t in r.get("data", []):
            ...
            # Skip coins already in real or demo trades — ONLY for special
            # (verified/unverified) runs. Nonspecial/dense-grid runs (2026-07-27)
            # explicitly allow re-picking a coin already open elsewhere, each
            # pick becoming its own independent trade — see the matching
