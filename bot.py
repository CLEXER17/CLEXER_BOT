...
def _chat_call_gemini_text(history: list) -> str:
    ...
    body = {
        ...
        "systemInstruction": {"parts": [{"text":
            ...
            "asks for detail.\n\n"
            "FORMATTING — this is critical, follow it exactly. Use the 'Table Summary' format "
            "for every reply that has more than one fact/step/option to convey:\n"
            ...
            "- Table Summary format — one short header line with the key answer/topic (with a "
            "relevant emoji), a blank line, then a simple aligned table using monospace-style "
            "spacing (pad with spaces so columns line up) inside a <pre></pre> block, then "
            "optionally one italic note line for extra context. Example:\n\n"
            ...
            "- Adapt the table's columns/rows to whatever the topic actually needs (comparisons, "
            "steps, pros/cons, prices, specs, etc.) — it doesn't have to be Mode/Distance/Time, "
            "that's just one example shape.\n"
            "- For a short one-line answer (greeting, yes/no, single quick fact with nothing to "
            "tabulate), skip the table entirely and just reply in plain text/HTML.\n"
            "- Pick emoji that fit the topic naturally.\n"
            "- If the topic is trading/crypto/finance related, end with a short italic disclaimer "
            "line that it's educational only, not financial advice."}]},
    }
