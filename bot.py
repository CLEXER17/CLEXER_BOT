...
ADMIN_CHAT_ID       = os.getenv("ADMIN_CHAT_ID",       "")   # YOUR personal Telegram chat ID
...
def build_smc_summary(data, ticker) -> str:
    ...
    for tf_key,(df,lb) in data.items():
        ...
        bos   = detect_bos_choch(df); sh,sl = find_swing_points(df,lb)
        lines += [f"--- {tf_key.upper()} | Trend: {trend} ---"]
...
You are an expert SMC trader. Charts: Weekly, 4H, 1H, 5M with full SMC annotations.

RULES:
- Weekly bias = king, never trade against it
- 4H confirms bias (BOS in trade direction)
- 1H has entry OB or FVG
- 5M shows entry momentum
- SL: last 4H swing HIGH (SHORT) or LOW (LONG)
- TP1: nearest opposing OB/FVG (min 1:2 R:R)
- TP2: next MAJOR swing level (min 1:4 R:R, 1500+ pts)
- Session: {get_session()}

You MUST give BUY or SELL. No NO_TRADE.
entry_type = MARKET (price at zone now) or PULLBACK (needs to return to zone).
...
{{
  ...
  "weekly_trend": "<weekly structure>",
  "structure_4h": "<4H key levels>",
  "entry_zone": "<OB/FVG range>",
...
WAIT — minor pullback
CLOSE — structure broken
NO_VOLUME — low volume"""
    try:
        msg = client.messages.create(
...
def send_reply(chat_id, text: str):
    """Reply directly to a user (for commands)"""
    try:
...
def handle_command(text: str, chat_id):
    ...
    # Security — only ADMIN can control bot
    if ADMIN_CHAT_ID and str(chat_id) != str(ADMIN_CHAT_ID):
        send_reply(chat_id, "⛔ Unauthorized. This is a private bot.")
        return
    ...
    # ── /status ────────────────────────────────────────────────────────────────
        ...
        send_reply(chat_id,
            ...
            f"Bot: {st}\n"
            f"Session: {se} {'✅' if is_trading_hours() else '⏸'}\n"
            f"IST: {ist_str()}\n"
    ...
    # ── /close ─────────────────────────────────────────────────────────────────
        ...
        if not t["signal"]:
            ...
            send_reply(chat_id, f"✅ Trade closed: {info}\nBot will find next trade on next scan.")
            force_scan.set()
    ...
    # ── /sltobe ────────────────────────────────────────────────────────────────
        ...
        if not t["signal"]:
            ...
            send_reply(chat_id, f"✅ SL moved to breakeven: {active_trade['entry']:,.0f}")
        ...
        if not t["signal"]:
            ...
            try:
                ...
                send_telegram(
                    f"🔄 <b>SL Updated</b>\n\n"
                    f"Old SL: {old_sl:,.0f}\n"
                    f"New SL: <b>{new_sl:,.0f}</b>\n\n"
                    f"<i>— CLEXER V3 —</i>"
                )
                send_reply(chat_id, f"✅ SL updated: {new_sl:,.0f}")
            except ValueError:
                send_reply(chat_id, "❌ Invalid price. Use: /setsl 61500")
    ...
    # ── /settp1 ────────────────────────────────────────────────────────────────
        ...
        if not t["signal"]:
            ...
            try:
                ...
                send_telegram(f"🔄 <b>TP1 Updated → {new_tp:,.0f}</b>\n\n<i>— CLEXER V3 —</i>")
                send_reply(chat_id, f"✅ TP1 updated: {new_tp:,.0f}")
            except ValueError:
                send_reply(chat_id, "❌ Invalid price. Use: /settp1 60000")
    ...
    # ── /settp2 ────────────────────────────────────────────────────────────────
        ...
        if not t["signal"]:
            ...
            try:
                ...
                send_telegram(f"🔄 <b>TP2 Updated → {new_tp:,.0f}</b>\n\n<i>— CLEXER V3 —</i>")
                send_reply(chat_id, f"✅ TP2 updated: {new_tp:,.0f}")
            except ValueError:
                send_reply(chat_id, "❌ Invalid price. Use: /settp2 58000")
        ...
        if len(parts) < 2:
            ...
            try:
                ...
                if hours < 1 or hours > 24:
                    send_reply(chat_id, "❌ Must be between 1–24 hours.")
                else:
                    SCAN_INTERVAL_SECONDS = int(hours * 3600)
                    send_reply(chat_id, f"✅ Scan interval set to {hours}h ({SCAN_INTERVAL_SECONDS}s)")
            except ValueError:
...
def main():
    ...
    while True:
        try:
            ...
            ist_now = now_ist()
            print(f"\n[{ist_now.strftime('%H:%M IST')}] Session: {get_session()}")
            ...
            if not is_trading_hours():
                print(f"  [WAIT] {get_session()} session — not London/NY")
                time.sleep(SCAN_INTERVAL_SECONDS)
                continue
            ...
            data = {}
            for key,interval,limit,lb in [("weekly","1w",52,5),("4h","4h",200,5),
                                           ("1h","1h",100,5),("5m","5m",50,3)]:
        ...
        # Smart sleep — wakes up instantly if /signal command received
        waited = 0
